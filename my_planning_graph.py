...
class PlanningGraph():
    ...
    def add_literal_level(self, level):
        ...
        for action_node in self.a_levels[level - 1]:
            for effnode in action_node.effnodes:
                s_node_new = effnode
                if effnode in self.s_levels[level]:
                    for s_node in self.s_levels[level]:
                        if effnode == s_node:
                            s_node_new = s_node
                            break
                s_node_new.parents.add(action_node)
                action_node.children.add(s_node_new)
                self.s_levels[level].add(s_node_new)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        negates = not set(node_a1.action.effect_add).isdisjoint(set(node_a2.action.effect_rem)) \
        or not set(node_a2.action.effect_add).isdisjoint(set(node_a1.action.effect_rem))
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        mutex = any(effect in node_a1.action.effect_rem for effect in node_a2.action.precond_pos) \
        or any(effect in node_a2.action.effect_rem for effect in node_a1.action.precond_pos)
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        negation = node_s1.symbol == node_s2.symbol and node_s1.is_pos != node_s2.is_pos
        return negation
